...
export async function POST(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        error: true,
        message:
          "postal code, city, address, are needed to continue this action ",
        data: null,
      },
      {
  ...
  if (lastPayment) {
    ...
    var orderId =
      `${moment.tz("Asia/Jakarta").format("YYYYMMDD")}web${formattedOrderNumber}`.toUpperCase();
  } else {
  ...
  var shipmentCreateDraft = await CreateDraftShipment(
    origin,
    user,
    couriers!,
    products
  ...
  //payment
  const pay = await PaymentDataCashless(
    total,
    orderId,
    tokenSuccess,
    tokenFailed
  ...
  const transactionData: Prisma.TransactionsCreateManyInput[] = products.map(
    (value, i) => {
      return {
